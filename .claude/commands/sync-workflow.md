...
Sync the ios-agent-workflow template repo so it stays consistent with your project's current conventions.
...
### 2. Read the template
- Read all files in the ios-agent-workflow template repo's `.claude/commands/`
...
```bash
git -C <ios-agent-workflow path> checkout develop && git -C <ios-agent-workflow path> pull
git -C <ios-agent-workflow path> checkout -b sync/<YYYY-MM-DD>
git -C <ios-agent-workflow path> add .claude/commands/
git -C <ios-agent-workflow path> commit -m "chore: sync commands from <AppName> — <brief summary>"
git -C <ios-agent-workflow path> push -u origin sync/<YYYY-MM-DD>
gh pr create --repo <owner>/ios-agent-workflow \
